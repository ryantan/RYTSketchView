...
// View Controllers
//#import "ClothsetClothsViewController.h"
//#import "ClothsetInfoViewController.h"
#import "UIImageView+ForScrollView.h"
#import "PenOptionsViewController.h"
...
typedef enum _WOEditorState {
    WOEDITOR_STATE_NEWWO,
    WOEDITOR_STATE_NEWWO_CUST_SELECTED,
    WOEDITOR_STATE_TEMPLATE_SELECTED,
    WOEDITOR_STATE_WO,
    WOEDITOR_STATE_CONNECTED_TO_INVOICE,
    WOEDITOR_STATE_CONFIRMED,
    WOEDITOR_STATE_LOCKED
} WOEditorState;
...
- (void)viewDidLoad {
    ...
    theSketchView =[[SketchView alloc]initWithFrame:self.view.bounds];
    ...
    joystickView = [[JoystickView alloc]initWithFrame:CGRectMake(10, 779, 120, 120)];
...
- (void)showPenOptionsFromButton:(UIButton*)button{
    ...
    PenOptionsViewController *penOptionsViewCtr = [[PenOptionsViewController alloc]init];
...
- (void)showClothsetOptions{
    ...
    ClothsetOptionsPopoverController *csOptionsViewCtr = [[ClothsetOptionsPopoverController alloc]init];
    csOptionsViewCtr.sketchViewController = self;
    mainPopoverController = [[UIPopoverController alloc]initWithContentViewController:csOptionsViewCtr];
    
    CGRect invokerRect = CGRectMake(btnCSOptions.frame.origin.x, btnCSOptions.frame.origin.y+35, btnCSOptions.frame.size.width, btnCSOptions.frame.size.height);
    [mainPopoverController presentPopoverFromRect:invokerRect inView:self.view permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
}
...
- (void)prepareSketchForAction:(NSString*)sketchAction userInfo:(NSDictionary *)userInfo{
    NSLog(@"SketViewController.prepareSketchForAction:\"%@\"userInfo:", sketchAction);
    
    //Clear previous options
    newWOOptions_workorderID = 0;
    newWOOptions_invoiceID = nil;
    newWOOptions_roleID = 0;
    newWOOptions_customerName = nil;
    
    if ([sketchAction isEqualToString:@"newWO"]){
        
    }else if ([sketchAction isEqualToString:@"editLocalWO"]){
        [[DataManager instance] loadWorkorder:(Workorder*)[userInfo objectForKey:@"workorder"]];
        
    }else if ([sketchAction isEqualToString:@"editRemoteWO"]){
        newWOOptions_workorderID = [[userInfo objectForKey:@"remoteWorkorderID"] integerValue];
        newWOOptions_invoiceID = [userInfo objectForKey:@"invoiceID"];
        newWOOptions_roleID = [[userInfo objectForKey:@"roleID"] integerValue];
        newWOOptions_customerName = [userInfo objectForKey:@"dispName"];
        
        [[DataManager instance] closeWorkorder];
...
- (void)showFabricPanel:(id)sender{
    ...
    if (isShowingClothsView){
        isShowingClothsView = NO;
    }else{
        isShowingClothsView = YES;
        [clothsViewController viewWillAppear:YES];
    }
...
- (void)showSketchOptionsPanel:(id)sender{
    ...
    if (_sketchOptionsPanel == nil) {
        _sketchOptionsPanel = [[SketchOptionsPanelController alloc] init];
        _sketchOptionsPanel.delegate = self;
        self.sketchOptionsPanelPopover = [[UIPopoverController alloc] initWithContentViewController:_sketchOptionsPanel];
    }
    ...
    CGRect sourceFrame = CGRectMake(btnSetOptions.frame.origin.x, btnSetOptions.frame.origin.y+44, btnSetOptions.frame.size.width, btnSetOptions.frame.size.height);
    
    [self.sketchOptionsPanelPopover presentPopoverFromRect:sourceFrame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
...
- (void)showWorkorderInfoPanel:(id)sender {
    ...
    if (_workorderInfoPanel == nil) {
        _workorderInfoPanel = [[WorkOrderInfoPanelController alloc] init];
        _workorderInfoPanel.delegate = self;
        _workorderInfoPanel.workorder = self.workorder;
        
        UINavigationController *workorderInfoNav = [[UINavigationController alloc]initWithRootViewController:_workorderInfoPanel];
        
        _workorderInfoPanelPopover = [[UIPopoverController alloc] initWithContentViewController:workorderInfoNav];
    }
    ...
    CGRect sourceFrame = CGRectMake(btnWOOptions.frame.origin.x, btnWOOptions.frame.origin.y+44, btnWOOptions.frame.size.width, btnWOOptions.frame.size.height);
    [self.workorderInfoPanelPopover presentPopoverFromRect:sourceFrame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
...
/*
- (void)loadWorkorder{
    [self loadWorkorder:[DataManager currentWorkorder]];
}
*/

/*
- (void)loadWorkorder:(Workorder *)wo{
    NSLog(@"SketchViewController.loadWorkorder");

    if (self.workorder == wo){
        return;
    }
    
    self.workorder = wo;
    
    [self updateTitleLabelWithCustomerName:self.workorder.customer workorder:self.workorder];
    
    [self initSetSelectorAnimated:YES];
    self.workorderInfoPanel = nil;
    self.workorderInfoPanelPopover = nil;
    self.sketchOptionsPanel = nil;
    self.sketchOptionsPanelPopover = nil;
    
    [clothsViewController reset];
    [theSketchView resetView];
...
- (void)setCustomer:(NSDictionary*)customer{
    [[DataManager instance] workorder:self.workorder setCustomer:customer];
    //lblTitle.text = [NSString stringWithFormat:@"Workder %d (%@)", self.currentWorkorder.localID, self.currentWorkorder.customer];
    [self updateTitleLabelWithCustomerName:self.workorder.customer workorder:self.workorder];
    if (_workorderInfoPanel){
        [_workorderInfoPanel reload];
    }
}
*/
...
- (void)updateTitleLabelWithCustomerName:(NSString*)customerName workorder:(Workorder*)theWorkorder{
    NSString *customerNameTruncated = [customerName stringByTruncatingToWidth:160 withFont:lblTitle.font];
    lblTitle.text = [NSString stringWithFormat:@"%@ for %@", theWorkorder.templateCategory, customerNameTruncated];
}
*/

/*
- (void)initSketchWithMeasureTemplateAnimated:(BOOL)animated{
    NSLog(@"initSketchWithMeasureTemplateAnimated:");
    ...
    templatePath = [[DataManager instance] getMeasurementTemplateByCategory:[self.workorder templateCategory]];
    NSLog(@"\ttemplatePath = %@",templatePath);
    
    // init template in SketchView
    ...
    // init sketch
    NSString *pathToSketch = [RYTSketchViewUtils getFilePathForWorkorder:self.workorder];
    ...
    [self didInitSketchWithMeasureTemplateAnimated:animated];
}
*/
...
- (void)initUIForWorkorderEditAnimated:(BOOL)animated{
    ...
    //lblClothsetID.text = [NSString stringWithFormat:@"Workorder:%@", self.workorder.localID];
    ...
    if (animated){
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            ...
            btnFabric.alpha = 0;   
            btnFabric.center = CGPointMake(btnFabric.center.x, 6);
            btnMeasure.alpha = 1;
            btnMeasure.center = CGPointMake(btnMeasure.center.x, 26);
            
            theSketchView.alpha = 1;
            
            viewOptionButtonsShirt.alpha = 1;
            viewOptionButtonsPants.alpha = 1;
            viewOptionButtonsJacket.alpha = 1;
...
//TODO: This is similar to initUIForWorkorderEditAnimated, consider refactor
/*
- (void)initUIForClothset:(NSUInteger)clothsetIndex animated:(BOOL)animated{
    NSLog(@"SketchViewController.initUIForClothset:Animated:");
    
    //TODO: Maybe change this to ref currentClothset object rather than the index?
    [DataManager setCurrentClothsetIndex:self.currentClothsetIndex];
    clothsViewController.clothset = [DataManager currentClothset];
    NSLog(@"clothsViewController.clothset=%@", clothsViewController.clothset);
    [clothsViewController reload];
    
    lblClothsetID.text = [NSString stringWithFormat:@"ClothsetID:%@", clothsViewController.clothset.localID];
    [self initSketchForClothset:clothsetIndex animated:animated];
    ...
    if (animated){
        ...
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            ...
            btnMeasure.alpha = 0;
            btnMeasure.center = CGPointMake(btnMeasure.center.x, 46);
            btnFabric.alpha = 1;
            btnFabric.center = CGPointMake(btnFabric.center.x, 26);
            btnPenRed.selected = FALSE;
            theSketchView.alpha = 1;
...
- (void)willSwitchEditMode{
    NSLog(@"willSwitchEditMode, currentSetSelectorIndex=%ld", (long)self.currentSetSelectorIndex);
    //btnSetSelector2.enabled = FALSE;
    [self saveChangesBeforeSwitchEditMode];
}
...
#pragma mark - RYTSketchViewDelegate
...
- (void)joystick:(JoystickView *)joystick moved:(CGPoint)delta{
